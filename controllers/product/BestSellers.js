...
const { TOP_SELLING_PRODUCT_URL, CART_URL, GET_CART_URL } = urls;

const BestSellersDetails = async (req, res) => {
    ...
    try {
        ...
        const TOP_BRAND = await axios.post(TOP_SELLING_PRODUCT_URL, {
        ...
        if (foundProduct) {
            res.render('product/BestSellers-Details', { userData: userData, product: foundProduct });
        } else {
...
const addToCartBestSellers = async (req, res) => {
    const userData = req.session.userData;
